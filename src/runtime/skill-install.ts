...
export async function installSkill(project: string, source?: Bundle): Promise<{ installed: string }> {
  ...
  try {
    ...
    for (const [path, content] of bundle) {
      ...
      if (path === 'SKILL.md') continue; // Activate only after helper and manifest exist.
      await mkdir(dirname(join(target, path)), { recursive: true });
      await writeFile(join(target, path), content, { flag: 'wx' });
    }
    await writeFile(join(target, MANIFEST), JSON.stringify({ schemaVersion: 1, owner: 'orqestra', files }, null, 2) + '\n', { flag: 'wx' });
...
export async function uninstallSkill(project: string): Promise<{ removed: string }> {
  ...
  let manifest: { schemaVersion?: unknown; owner?: unknown; files?: unknown };
  try { manifest = JSON.parse(await readFile(join(target, MANIFEST), 'utf8')) as typeof manifest; }
  catch { throw new InputError('Cannot remove skill: ownership manifest is missing or invalid'); }
  if (!manifest || manifest.schemaVersion !== 1 || manifest.owner !== 'orqestra' || !manifest.files || typeof manifest.files !== 'object' || Array.isArray(manifest.files)) {
    throw new InputError('Cannot remove skill: unrecognized ownership manifest');
  }
  const entries = Object.entries(manifest.files);
  if (!entries.some(([path]) => path === 'SKILL.md') || !entries.some(([path]) => path === 'scripts/runtime/cli.js')) throw new InputError('Cannot remove skill: incomplete ownership manifest');
  const expected = [...entries.map(([path]) => path), MANIFEST].sort();
  if (JSON.stringify(actual) !== JSON.stringify(expected)) throw new InputError('Cannot remove skill: files were added or removed; preserve or review local changes first');
  for (const [path, hash] of entries) {
    safeRelative(path);
    if (typeof hash !== 'string' || !/^[a-f0-9]{64}$/u.test(hash) || digest(await readFile(join(target, path.split('/').join(sep)))) !== hash) {
      throw new InputError(`Cannot remove skill: modified artifact ${path}; preserve or review local changes first`);
    }
  }
