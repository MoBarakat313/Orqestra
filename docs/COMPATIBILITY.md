...
| Public alpha archive | `0.1.0-alpha.1`; Node.js 22; GitHub Ubuntu, macOS, and Windows runners plus local macOS ARM64 | Packed allowlisted files installed without lifecycle scripts or source-checkout dependencies; version, setup, bundled demo, status, staged upgrade, removal, retained policy, and content checks passed. |
