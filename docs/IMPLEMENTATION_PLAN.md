...
| M6 — Accounting and evaluation | Token categories, coordinator visibility/gaps, distinct API/account reports, reproducible direct-vs-Orqestra benchmark | Same initial repo/task conditions; quality and retries reported; no unexecuted counterfactual presented as savings | Complete — 90 offline tests cover telemetry, persistence, account boundaries, and paired evaluation; real observations remain explicitly unverified |
| M7 — Public alpha distribution | Guided setup, versioned packages, diagnostics, reversible installation, configuration migration, compatibility matrix | Clean-machine install and removal verified on supported platforms; all documented commands work; releases contain no private data | Complete — 99 tests and archive lifecycle checks pass on Linux, macOS, and Windows; `0.1.0-alpha.1` release automation is ready |
...
- M6: added App Server token-category parsing, fresh-thread totals, cumulative repair deltas, explicit missing/interrupted telemetry gaps, durable and coordinated aggregation, zero-model-turn account inspection, and separate ChatGPT/API-key reports. Added a strict paired benchmark ledger that binds repetitions to one task contract and base commit, reports quality/regressions/retries/time, excludes incomplete pairs, and emits token or API-cost differences only from matched measured observations. Offline fixtures validate these boundaries; no real M6 usage observation or savings benchmark is claimed.
...
- Post-M7 beginner experience: added a one-prompt verified installation path, linked manual alternative, beginner command guide, complete alpha command index, and skill guidance for focused model-role and limit edits followed by strict validation. The current CLI still has no interactive `configure` command; that behavior is not claimed.
- Post-M7 public explanation: documented the Safe Token strategy as configurable routing rather than a fixed savings claim, added an original Astra versus routed-work illustration with an explicit unmeasured-pattern caption, and improved README scanning with restrained section icons.
- Post-M7 public explanation: added an activation and routing diagram, route and worker-control examples, current model-selection boundaries, and a mechanism-based account of intended token reduction. The README retains the explicit statement that no fixed savings percentage has been established.

## After the alpha

Use public-alpha feedback and paired benchmark evidence to choose the next milestone. Priorities are the unverified real-runtime recovery and cancellation paths in [COMPATIBILITY.md](COMPATIBILITY.md), clearer natural-language contract authoring inside Codex, a deterministic guided `configure` command, and additional runtime adapters only when they can be tested end to end.
