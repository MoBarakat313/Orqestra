...
M4: persist run state without storing credentials or unrestricted backend logs. Add typed failure and checkpoint transitions, bounded repair attempts using the configured `maxAttempts`, resume after process interruption without duplicating worker side effects, and cancellation cleanup. Verification failure must retain evidence and useful edits while preventing a successful terminal state.
