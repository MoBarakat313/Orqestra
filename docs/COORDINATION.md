...
Contracts, command arrays, verifier output, agent messages, credentials, approval details, and backend logs are excluded from the aggregate checkpoint. Usage remains `null`; M5 enforces dispatch counts but does not infer tokens or cost.
